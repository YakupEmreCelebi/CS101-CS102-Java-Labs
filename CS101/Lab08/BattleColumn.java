...
public class BattleColumn {
     ...
     */
    private char [] elements = new char[10];
    ...
    {
        for(int i=0; i<1 + (int)(Math.random() * SkyBattleGame.MAX_DARK_KNIGHT_SIZE); i++)
        {
            elements[(int)(Math.random() * SkyBattleGame.ROWS)] = SkyBattleGame.DARK_KNIGHT_SYMBOL;
        }
        if((int)  Math.round(Math.random())  == 1)
        {
            elements[(int)(Math.random() * SkyBattleGame.ROWS)] = 'X';
        }
    }
